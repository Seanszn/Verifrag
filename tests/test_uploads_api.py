...
from src.api.dependencies import database
...
def test_upload_requires_authentication(tmp_path: Path):
...
def test_upload_ingests_text_file_to_user_workspace(monkeypatch, tmp_path: Path):
    ...
    test_db.initialize()
    database.path = test_db.path

    user = database.create_user("alice", hash_password("password123"))
