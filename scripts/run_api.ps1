...
if (-not $env:ENABLE_VERIFICATION) { $env:ENABLE_VERIFICATION = "false" }
...
Write-Host "DEPLOYMENT_MODE=$env:DEPLOYMENT_MODE API_HOST=$env:API_HOST API_PORT=$env:API_PORT ENABLE_VERIFICATION=$env:ENABLE_VERIFICATION"

& $VenvPython -m uvicorn src.api.main:app --host $env:API_HOST --port $env:API_PORT
