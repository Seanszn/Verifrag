...
from src.retrieval.case_targeting import filter_search_hits_to_target_case
...
class BM25Retriever:
    ...
    def retrieve(self, query: str, k: int = 10) -> list[LegalChunk]:
        ...
        raw_hits = self.index.search(query, k=candidate_limit)
        hits = filter_search_hits_to_target_case(
            query,
            raw_hits,
            metadata_case_name=lambda item: item[2].get("case_name") if len(item) == 3 else None,
            limit=limit,
        )
...
def retrieve_bm25_hits(
    ...
    *,
    ...
    raw_hits = index.search(query, k=max(top_k * 6, top_k))
    raw_hits = filter_search_hits_to_target_case(
        query,
        raw_hits,
        metadata_case_name=lambda item: item[2].get("case_name") if len(item) == 3 else None,
        limit=top_k,
